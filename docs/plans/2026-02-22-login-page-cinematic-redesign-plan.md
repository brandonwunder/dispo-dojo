# Login Page Cinematic Redesign — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the Three.js login scene with a photo-first cinematic experience using CSS zoom animation, atmospheric layers, and glassmorphism login/sign-up cards.

**Architecture:** Remove DojoGateScene from Login.jsx. Build a composited 2D layer stack (background photo → vignette → rain canvas → fog → lantern glow → login card). All animation driven by CSS keyframes + Framer Motion. Reuse existing RainEffect component with a prop for reduced particle count.

**Tech Stack:** React 19, Framer Motion, CSS animations, existing RainEffect canvas component, Tailwind CSS v4

---

### Task 1: Add `count` prop to RainEffect for reuse on login

**Files:**
- Modify: `frontend/src/components/RainEffect.jsx:14` (RAIN_COUNT constant)

**Step 1: Add optional `count` prop with default**

In `frontend/src/components/RainEffect.jsx`, change the component signature and constant:

```jsx
// Line 3: Change signature
export default function RainEffect({ count = 280, lightning = true }) {

// Line 14: Use prop
const RAIN_COUNT = count

// Line 120-127: Wrap lightning trigger in conditional
if (lightning && !ltActive && t >= ltNext) {
```

This lets Login pass `<RainEffect count={150} lightning={false} />` for a calmer rain.

**Step 2: Verify dashboard still works**

Run: `cd frontend && npx vite` — load dashboard, confirm rain + lightning still renders at 280 drops.

**Step 3: Commit**

```bash
git add frontend/src/components/RainEffect.jsx
git commit -m "feat: add count and lightning props to RainEffect for login reuse"
```

---

### Task 2: Rewrite Login.jsx — remove Three.js, build layer stack

**Files:**
- Modify: `frontend/src/pages/Login.jsx` (full rewrite of the render section)

**Step 1: Replace imports**

Remove the DojoGateScene import (line 5). Add RainEffect import:
...
**Step 2: Add animation state for cinematic entrance**

After the existing state declarations (lines 216-222), add:
...
**Step 3: Rewrite the JSX return — layer stack**

Replace the entire return block (lines 251-346) with the new composited layer stack:
...
git commit -m "feat: replace Three.js login scene with cinematic photo-first layer stack"
```

---

### Task 3: Add CSS keyframes for fog drift and lantern pulse

**Files:**
- Modify: `frontend/src/index.css` (add keyframes after existing `@keyframes logoFloat` block, ~line 721)

**Step 1: Add keyframes**

Insert after the `logoFloat` keyframe block:

```css
/* Fog drift animations for login scene */
@keyframes fogDrift1 {
...
/* Lantern glow pulse for login scene */
...
git commit -m "feat: add fog drift and lantern pulse keyframes for login scene"
```

---

### Task 4: Redesign SignUpModal with glassmorphism

**Files:**
- Modify: `frontend/src/pages/Login.jsx` (SignUpModal function, lines 15-213)

**Step 1: Rewrite the SignUpModal JSX**

Replace the entire return block of SignUpModal (lines 63-212) with glassmorphism styling:
            ...
            {step === 1 && (
              ...
              >
                <div className="text-center mb-8">
                  <h2 className="font-display text-2xl text-[#F4F7FA] mb-2 tracking-[0.06em] gold-shimmer-text">
                    Make an Account
                  </h2>
...
```bash
git add frontend/src/pages/Login.jsx
git commit -m "feat: redesign sign-up modal with glassmorphism matching login card"
```

---

### Task 5: Screenshot, compare, and polish

**Step 1: Screenshot login page**

```bash
node screenshot.mjs http://localhost:3015/login redesigned-login
```

Read the screenshot, compare against the design doc specs. Check:
- Background photo zoom-in animation plays on load
- Glassmorphism card is visible with blur showing scene through it
- Gold shimmer on "Enter the Dojo" heading
- Input focus states have cyan glow
- Red CTA button with proper gradient
- Rain falls without lightning
- Fog blobs drift gently
- Lantern glows pulse at correct positions

**Step 2: Screenshot sign-up modal**

Click "Make an Account for FREE", then screenshot:
```bash
node screenshot.mjs http://localhost:3015/login signup-modal
```

Check:
- Blurred overlay behind modal
- Glassmorphism card matches login card style
- Step indicator with cyan active dots
- Step transition animation works (left/right slide)

**Step 3: Fix any visual issues found**

Address spacing, color, or animation mismatches.

**Step 4: Final commit**

```bash
git add -A
git commit -m "fix: polish login page visual details after screenshot review"
```
